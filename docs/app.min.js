...
class CrisisDashboard {
  ...
  clearLoadingIfStuck(timeoutMs = 5000) {
    ...
    setTimeout(() => {
      try {
        ...
        // Also check computed style in case inline style didn’t update
        ...
        if (actuallyVisible) {
          ...
          if (errorEl && errorEl.style.display !== 'block') {
            ...
            errorEl.innerHTML = '<p>❌ Loading took too long. Please try again.</p>';
          }
        }
      } catch (_) {}
    }, timeoutMs);
  ...
  renderTicker() {
    ...
    if (!items.length) {
      ...
      const altHtml = alt.map(i => `<span class="ticker-item"><a href="${this.escapeHtml(i.url || '#')}" target="_blank" rel="noopener noreferrer">${this.escapeHtml(i.label || '')}</a><span class="ticker-sep">•</span></span>`).join('');
      ...
      if (widenBtn) {
        widenBtn.onclick = () => {
          ...
          if (next.length) {
            ...
            bar.style.display = 'block';
            document.body.classList.add('ticker-active');
            const html = next.map(i => `<span class="ticker-item"><a href="${this.escapeHtml(i.url || '#')}" target="_blank" rel="noopener noreferrer">${this.escapeHtml(i.label || '')}</a><span class="ticker-sep">•</span></span>`).join('');
            ...
            const altHtml2 = alt2.map(i => `<span class=\"ticker-item\"><a href=\"${this.escapeHtml(i.url || '#')}\" target=\"_blank\" rel=\"noopener noreferrer\">${this.escapeHtml(i.label || '')}</a><span class=\"ticker-sep\">•</span></span>`).join('');
      ...
      if (resetBtn) {
        resetBtn.onclick = () => {
          ...
          if (!next.length) {
            ...
            bar.style.display = 'block';
            document.body.classList.add('ticker-active');
            const html2 = next.map(i => `<span class=\"ticker-item\"><a href=\"${this.escapeHtml(i.url || '#')}\" target=\"_blank\" rel=\"noopener noreferrer\">${this.escapeHtml(i.label || '')}</a><span class=\"ticker-sep\">•</span></span>`).join('');
  ...
  getFallbackTickerItems(windowHours = 2) {
    ...
    const msgs = [
      `⏰ ${nowStr} • Continuous Monitoring Active. No Critical Events in the last ${windowHours} Hours.`,
  ...
  updateSpeedLabel() { const label = document.getElementById('tickerSpeedLabel'); if (!label) return; const v = this.getDisplaySpeed(); let emoji = ''; if (v > 80) emoji = '🐇'; else if (v < 20) emoji = '🐢'; label.innerHTML = `Speed: ${v}${emoji ? ' <span class=\"speed-emoji\">'+emoji+'</span>' : ''}`; }
  ...
  showStockInfo(message, type = 'info') {
    ...
    const timeEl = document.getElementById('stockInfoTime');
      ...
      const nextMsg = mins ? ` • Next refresh in ${mins} min` : '';
      timeEl.textContent = `${now.toLocaleString()} 🕒 Last Updated${nextMsg}`;
    }
  }
  ...
  formatPrice(n) { if (n == null || Number.isNaN(Number(n))) return '—'; return Number(n).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  formatPct(n) {
